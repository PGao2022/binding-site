...
'''
def residue_encoding(self, res, overlap=True):
    residue_types = ['ALA','CYS','ASP','GLU','PHE','GLY','HIS','ILE','LYS','LEU','MET','ASN','PRO','GLN','ARG','SER','THR','VAL','TRP','TYR','XAA']
    res = seq[:3]
    with open("kalasanty/%s.json" % self.encoding_type, 'r') as load_f:
...
class DataWrapper():
    ...
    def prepare_complex(self, pdbid, rotation=0, translation=(0, 0, 0),
        ...
        rec_coords = tfbio1.data.rotate(structure['coords'][:], rotation)
        rec_coords += translation
        rec_grid = tfbio1.data.make_grid(rec_coords, structure['features'][:],
                                        max_dist=self.max_dist,
                                        grid_resolution=resolution)

        pocket_coords = tfbio1.data.rotate(structure['pocket_coords'][:],
                                          rotation)
        pocket_coords += translation
        pocket_dens = tfbio1.data.make_grid(pocket_coords,
