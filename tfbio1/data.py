...
class Featurizer():
    ...
    def __init__(self, atom_codes=None, atom_labels=None,
                 ...
                 smarts_labels=None, encode_residue = None):
        ...
        for i, atom in enumerate(molecule):
            ...
            if atom.atomicnum > 1:
                ...
                for prop in self.NAMED_PROPS:
                    temp = atom.__getattribute__(prop)
                    #temp = self.encode_num(atom.atomicnum)
                    print(temp)
                    import pdb
                    pdb.set_trace()
                '''
                ...
                features.append(np.concatenate((
                    self.encode_num(atom.atomicnum),
                    [atom.__getattribute__(prop) for prop in self.NAMED_PROPS],
                    np.array([func(atom) for func in self.CALLABLES]).flatten()
                )))
